...
from pm_nba_agent.shared import Channels, RedisClient
...
class Signal:
    ...
    type: str                      # 信号类型（如 "scoreboard", "strategy_signal"）
    source: str                    # 发送方机器人类型
    task_id: str                   # 所属任务 ID
...
class BaseRobot(ABC):
    ...
    async def run_loop(self) -> None:
        ...
        无 input_streams 的机器人依靠 setup() 中启动的异步任务驱动，
    ...
    async def save_snapshot(self, name: str, data: dict[str, Any], ex: int | None = None) -> None:
        """将静态数据写入 Redis snapshot key（一写多读）。"""
        key = Channels.task_snapshot(self.task_id, name)
        payload = json.dumps(data, ensure_ascii=False)
        if ex is not None:
            await self.redis.set(key, payload, ex=ex)
        else:
            await self.redis.set(key, payload)
        logger.debug("Robot {} save_snapshot: key={}", self.robot_type, key)

    async def load_snapshot(self, name: str, timeout: float = 10.0) -> dict[str, Any]:
        """从 Redis snapshot key 读取 JSON；若 key 不存在则轮询等待。"""
        key = Channels.task_snapshot(self.task_id, name)
        ...
        while True:
            ...
            if asyncio.get_event_loop().time() >= deadline:
                logger.warning(
                    "Robot {} load_snapshot 超时: key={} timeout={}s",
                    self.robot_type, key, timeout,
                )
                return {}
